...
<ul class="navbar-nav" style="font-size: 17px;">
            ...
            <li class="nav-item">
             ...
             <li class="nav-item active">
                <a class="nav-link" href="#">Add Review Report</a>
            </li>
            <li class="nav-item">
            ...
            if(mysqli_num_rows($run)>0){
                ...
                <?php
                 while($res=mysqli_fetch_assoc($run))
                        {    
                           if (empty($res['sem1_rev1'])||empty($res['sem1_rev2'])||empty($res['sem1_rev3'])||empty($res['sem2_rev1'])||empty($res['sem2_rev2'])||empty($res['sem2_rev3'])) {
                            ?><tr > 

                                <td style="font-size: 20px;" ><?php echo $res['proj_id'] ;?></td>
                                <td style="font-size: 20px;"><?php echo $res['project_title'] ;?>
                                </td>
                                <?php
                                if (empty($res['sem1_rev1'])||empty($res['sem1_rev2'])||empty($res['sem1_rev3'])) {
                                ?>
                                <td style=""><button type="button" class="btn btn-outline-info waves-effect" onclick="window.location='review_form.php?id=<?php echo $res['proj_id'];?>'">SEMESTER 7</button>
                                </td>
                                <?php
                                 }
                                 else
                                ...
                                ?>
                                 <td style=""><button type="button" class="btn btn-outline-info waves-effect" onclick="window.location='review_form_sem2.php?id=<?php echo $res['proj_id'];?>'">SEMESTER 8</button>
                                </td>
                                <?php
                                 }
                                 else
...
<footer class="page-footer" style="background-color: rgba(51, 187, 255,0.6);bottom: 0px;position: fixed;width: 100%;z-index: 999;">
